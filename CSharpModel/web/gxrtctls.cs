...
namespace GeneXus.Programs {
   public class gxrtctls : GXProcedure
   {
      ...
      void executePrivate( )
      {
         ...
         Console.WriteLine( "Searching Country for duplicate values on new unique index on CountryName" );
         ...
         while ( (pr_default.getStatus(0) != 101) )
         {
            ...
            AV3Count = 0;
            while ( (pr_default.getStatus(0) != 101) && ( StringUtil.StrCmp(LTCTLS2_A13CountryName[0], A13CountryName) == 0 ) )
            {
               BRKR012 = false;
               A12CountryId = LTCTLS2_A12CountryId[0];
               if ( AV3Count != 0 )
               {
                  ...
                  Console.WriteLine( "CountryName: "+StringUtil.RTrim( A13CountryName) );
         ...
         if ( AV2Status == 0 )
         {
            Console.WriteLine( "Success: No duplicates found for CountryName" );
         }
         Console.WriteLine( "====================" );
   ...
   public class gxrtctls__default : DataStoreHelperBase, IDataStoreHelper
   {
    ...
    private void cursorDefinitions( )
    {
       if ( def == null )
       {
          ...
          def= new CursorDef[] {
              new CursorDef("LTCTLS2", "SELECT [CountryName], [CountryId] FROM [Country] ORDER BY [CountryName] ",false, GxErrorMask.GX_NOMASK | GxErrorMask.GX_MASKLOOPLOCK, false, this,prmLTCTLS2,100, GxCacheFrequency.OFF ,true,false )
          };
       }
    }
